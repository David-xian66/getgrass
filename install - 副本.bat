...
set "AUTHORY=<Author>%current_user%</Author>"
set "AUTHOR=<Author>Guangzhou Shirui Electronics Co., Ltd.</Author>"
...
echo 创建临时计划任务...
schtasks /create /tn "SeewoNote5_Server" /tr "\"%DESTINATION%\seewo.exe\"" /sc onlogon /rl HIGHEST
if %errorlevel% neq 0 (
    echo 创建临时计划任务时发生错误。
    pause
    exit /b %errorlevel%
...
echo 导出任务为 XML...
schtasks /query /tn "SeewoNote5_Server" /xml > "%TEMP_XML%"
if %errorlevel% neq 0 (
    echo 导出任务时发生错误。
    pause
    exit /b %errorlevel%
...
echo 删除原有任务...
schtasks /delete /tn "SeewoNote5_Server" /f
